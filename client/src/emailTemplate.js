// Builds the same HTML summary email as server/mailer.js's generateTaskEmailHTML,
// but entirely client-side (used for the live preview — no server round-trip).
// Keep this in sync with server/mailer.js if the template changes on either side.
...
export function generateTaskEmailHTML(pendingTasks, scheduledTime) {
        ...
        <div style="background-color: #ea4335; padding: 24px 30px; color: #ffffff; display: flex; align-items: center; justify-content: space-between;">
          <div>
            ...
            <p style="margin: 6px 0 0 0; font-size: 14px; opacity: 0.9;">
              Gmail Task Manager • Envío automático (${scheduledTime || '08:00'} hrs)
            </p>
          </div>
        </div>
