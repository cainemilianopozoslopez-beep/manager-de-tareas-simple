...
**Notifications are browser-only.** The `Notification` API path (client-side, gated on permission, polled every 10s against `settings.scheduledTime`) is the only working delivery mechanism. Gmail/SMTP-based email sending was removed entirely (mid-2026) — it never worked (there was no server to hold credentials safely) and was letting users type a real Gmail app password into a field that got stored in Firestore in plain text for a feature that always failed on send. `EmailPreviewModal`, `emailTemplate.js`, the "Correo Gmail"/"Ambos" notification modes, and the `senderEmail`/`senderPass`/`recipientEmail` settings fields are all gone. If email sending gets built again, it needs a serverless function (Vercel/Firebase Cloud Function) to hold credentials server-side — don't resurrect a client-stored password field.
